...
enum Action {NONE, IDLE, FALLING, WALKING, JUMPING, DOUBLE_JUMPING, WALL_SLIDING_UP, WALL_SLIDING_DOWN, WALL_JUMPING}
...
func _is_jumping() -> bool:
	...
	return _action == Action.JUMPING or _action == Action.DOUBLE_JUMPING or _action == Action.WALL_JUMPING
...
func _determine_has_jumped_or_stopped() -> void:
	if Input.is_action_just_pressed("ui_accept"):
		_has_jumped = is_on_wall_only() or is_on_floor() or _action == Action.JUMPING
	else:
		_has_jumped = false
...
func _determine_action() -> Action:
	...
	if is_on_wall_only() and _has_jumped:
		return Action.WALL_JUMPING
	if _has_jumped and _action == Action.JUMPING:
		return Action.DOUBLE_JUMPING
	if _has_jumped:
		return Action.JUMPING
	if is_on_wall_only():
		return Action.WALL_SLIDING_DOWN if velocity.y > 0.0 else Action.WALL_SLIDING_UP
...
func _animate_action() -> void:
	...
	match _action:
		...
		Action.JUMPING, Action.DOUBLE_JUMPING, Action.FALLING:
			$AnimatedSprite2D.play("jump")
		Action.WALL_SLIDING_DOWN:
			$AnimatedSprite2D.play("wall_slide_down")
		Action.WALL_SLIDING_UP:
